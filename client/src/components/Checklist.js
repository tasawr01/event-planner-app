...
const Checklist = () => {
  ...
  const handleToggleCompleted = async (itemId, currentStatus) => {
    try {
      // Call the API to toggle the completion status
  ...
  const handleEditChecklist = (itemId, currentTitle) => {
    ...
    if (newTitle && newTitle.trim() !== currentTitle) {
      try {
        ...
        API.put(`/checklists/checklist/${itemId}/edit`, { title: newTitle })
          .then((response) => {
            setChecklists((prev) =>
              prev.map((item) =>
                item._id === itemId ? { ...item, title: response.data.title } : item
              )
            );
          })
          .catch((err) => console.error("Error editing checklist item:", err));
      } catch (err) {
